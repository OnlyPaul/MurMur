...
const ModelCard: React.FC<ModelCardProps> = ({
  ...
  const baseClasses =
    "flex flex-col rounded-[10px] px-4 py-[14px] gap-[14px] text-left transition-colors duration-200";

  const getVariantClasses = () => {
    if (status === "active") {
      return "border border-[var(--hairline-strong)] bg-[var(--surface-elevated)]";
    }
    if (isFeatured) {
      return "border border-[var(--hairline)] bg-[var(--surface)]";
    }
    return "border border-[var(--hairline)] bg-[var(--surface)]";
  };
  ...
  const getInteractiveClasses = () => {
    ...
    return "cursor-pointer hover:border-[var(--hairline-strong)] hover:bg-[var(--surface-elevated)] group";
  };
      ...
      <div className="flex items-center gap-3 w-full -mb-0.5 mt-0.5 h-5">
        ...
        {onDelete && (status === "available" || status === "active") && (
          <Button
            ...
            className="flex items-center gap-1.5 ms-auto text-[var(--signal)] hover:text-[var(--signal-pressed)] hover:bg-[var(--signal-soft)]"
          >
