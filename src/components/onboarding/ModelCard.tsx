...
const ModelCard: React.FC<ModelCardProps> = ({
  ...
  const getVariantClasses = () => {
    if (status === "active") {
      return "border-2 border-signal/50 bg-signal/10";
    }
    if (isFeatured) {
      return "border-2 border-signal/25 bg-signal/5";
    }
    return "border-2 border-mute/20";
  };

  const getInteractiveClasses = () => {
    if (!isClickable) return "";
    if (disabled) return "opacity-50 cursor-not-allowed";
    return "cursor-pointer hover:border-signal/50 hover:bg-signal/5 hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] group";
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center w-full">
        <div className="flex flex-col items-start flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            <h3
              className={`text-base font-semibold text-ink ${isClickable ? "group-hover:text-signal" : ""} transition-colors`}
            >
              {displayName}
            </h3>
          ...
          <div className="hidden sm:flex items-center ms-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <p className="text-xs text-ink/60 w-24 text-end">
                  {t("onboarding.modelCard.accuracy")}
                </p>
                <div className="w-16 h-1.5 bg-mute/20 rounded-full overflow-hidden">
                  <div
      ...
      <hr className="w-full border-mute/20" />
      ...
      <div className="flex items-center gap-3 w-full -mb-0.5 mt-0.5 h-5">
        ...
        {status === "downloadable" && (
          <span className="flex items-center gap-1.5 ms-auto text-xs text-ink/50">
        ...
        {onDelete && (status === "available" || status === "active") && (
          <Button
            ...
            className="flex items-center gap-1.5 ms-auto text-signal/85 hover:text-signal hover:bg-signal/10"
          >
      ...
      {status === "downloading" && downloadProgress !== undefined && (
        <div className="w-full mt-3">
          <div className="w-full h-1.5 bg-mute/20 rounded-full overflow-hidden">
            <div
      ...
      {status === "verifying" && (
        <div className="w-full mt-3">
          <div className="w-full h-1.5 bg-mute/20 rounded-full overflow-hidden">
            <div className="h-full bg-signal rounded-full animate-pulse w-full" />
          </div>
